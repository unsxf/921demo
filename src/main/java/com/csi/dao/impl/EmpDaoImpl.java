...
public class EmpDaoImpl implements EmpDao {
    ...
    public Emp selectById(int id) throws SQLException {
        ...
        Connection conn = DBUtil.getConnection();
        //获取数据库操作对象
        PreparedStatement pstmt = DBUtil.getPstmt(conn, "select * from emp where empno=?");
        //执行sql语句
        ResultSet rs = DBUtil.query(pstmt);
    ...
    @Override
    public Page<Emp> selectByPage(int pageNum, int pageSize) throws SQLException {
        return null;
    }
...
//        Connection conn = DBUtil.getConnection();
        //获取数据库操作对象
//        PreparedStatement pstmt = DBUtil.getPstmt(conn, "select * from emp e,dept d where e.deptno = d.deptno limit ?,?",
//                page.start(), page.getPageSize());
        //执行sql语句
//        ResultSet rs = (ResultSet) new ArrayList<Emp>();
//        while (rs.next()){
////            list.add(new Emp(rs.getInt("empno"),
//                    rs.getString("ename"),
//                    rs.getString("job"),
